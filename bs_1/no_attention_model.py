from keras.models import Model
from keras.layers import Input, GRU, Dense, Multiply, Attention
from keras.callbacks import EarlyStopping, ModelCheckpoint
import numpy as np
import os
from keras.layers import Bidirectional, Conv1D, BatchNormalization, Dropout
from keras.layers import Dot, Concatenate, Activation  # 用于自定义注意力机制
from keras.optimizers import Adam  # 替换原有优化器
from keras.metrics import RootMeanSquaredError  # 新增评估指标
def build_baseline_model(input_shape):  # 重命名函数以示区别
    inputs = Input(shape=input_shape)
    
    # 双向GRU保持不变
    gru_out = Bidirectional(GRU(128, return_sequences=True))(inputs)
    
    # 直接使用GRU输出（跳过注意力机制部分）
    conv_input = gru_out  # 变量重命名保持一致性
    
    # 时间序列特征提取
    conv1 = Conv1D(64, 3, activation='relu', padding='same')(conv_input)
    conv1 = BatchNormalization()(conv1)
    
    # 输出层保持不变
    output = Dense(64, activation='relu')(conv1[:, -1, :])
    output = Dropout(0.4)(output)
    output = Dense(1)(output)
    
    model = Model(inputs=inputs, outputs=output)
    model.compile(optimizer=Adam(learning_rate=0.00009), 
                 loss="mse",
                 metrics=[RootMeanSquaredError()])
    return model

def train_model(symbol):
    # 加载数据
    data_path = f"bs_1/processing_data/{symbol}"
    X_train = np.load(f"{data_path}/X_train.npy")
    y_train = np.load(f"{data_path}/y_train.npy")
    # 训练验证分割
    val_split = int(0.2 * len(X_train))
    X_train = X_train[:-val_split]
    y_train = y_train[:-val_split]
    X_val = X_train[val_split:]
    y_val = y_train[val_split:]
    # 模型参数
    input_shape = (X_train.shape[1], X_train.shape[2])
    model = build_baseline_model(input_shape)
    
    # 训练配置
    callbacks = [
        EarlyStopping(patience=20, restore_best_weights=True),
        ModelCheckpoint(f"bs_1/model/{symbol}_no_attention_best.h5", save_best_only=True)
    ]
    
    # 训练模型
    history = model.fit(
        X_train, y_train,
        validation_data=(X_val, y_val),
        epochs=100,
        batch_size=32,
        callbacks=callbacks,
        verbose=2
    )
# if __name__ == "__main__":
#     symbols = [f.split(".")[0] for f in os.listdir("bs_1/processing_data")]
#     for symbol in symbols:
#         print(f"正在处理 {symbol}...")
#         train_model(symbol)
